...
namespace mpc {
  ...
  template <typename T>
  [[nodiscard]] constexpr auto forward_like(auto&& x) noexcept -> __forward_like_t<T, decltype(x)> {
...
namespace mpc {
  ...
  constexpr auto __make_partial_impl(Op&& op, Tuple&& tuple, std::index_sequence<Idx...>, Args&&... args) noexcept(
  ...
  constexpr auto __make_partial(Op&& op, Args&&... args) noexcept(
    noexcept(   __make_partial_impl(forward_like<Op>(op.op_), forward_like<Op>(op.bound_), std::make_index_sequence<std::tuple_size_v<decltype(op.bound_)>>(), std::forward<Args>(args)...)))
    -> decltype(__make_partial_impl(forward_like<Op>(op.op_), forward_like<Op>(op.bound_), std::make_index_sequence<std::tuple_size_v<decltype(op.bound_)>>(), std::forward<Args>(args)...)) {
    return      __make_partial_impl(forward_like<Op>(op.op_), forward_like<Op>(op.bound_), std::make_index_sequence<std::tuple_size_v<decltype(op.bound_)>>(), std::forward<Args>(args)...);
  }
  ...
  private:
    ...
    friend constexpr auto __make_partial(Op2&&, Args&&...);
